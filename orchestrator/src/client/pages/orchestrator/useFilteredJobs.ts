...
import {
  DEFAULT_DISCOVERY_AGE_FILTER,
  DISCOVERY_ACTION_QUEUE_LIMIT,
} from "./constants";
...
export const useFilteredJobs = (
  ...
  sort: JobSort,
  discoveryAgeFilter: DiscoveryAgeFilter = DEFAULT_DISCOVERY_AGE_FILTER,
) =>
  useMemo(() => {
    let filtered = [...jobs];
    ...
    if (activeTab === "ready") {
      ...
      filtered = applyDiscoveryAgeFilter(filtered, discoveryAgeFilter);
    } else if (activeTab === "applied") {
      filtered = filtered.filter((job) => job.status === "applied");
    } else if (activeTab === "archived") {
      filtered = filtered.filter(
        (job) => job.status === "expired" || job.status === "skipped",
      );
    ...
    if (
      ...
      sorted.length > DISCOVERY_ACTION_QUEUE_LIMIT
    ) {
      return sorted.slice(0, DISCOVERY_ACTION_QUEUE_LIMIT);
    }
    return sorted;
  }, [
    jobs,
...
const applyDiscoveryAgeFilter = (
  jobs: JobListItem[],
  discoveryAgeFilter: DiscoveryAgeFilter,
): JobListItem[] => {
  if (discoveryAgeFilter === "all") {
    return jobs.filter((job) => getDeadlineUrgency(job) !== "expired");
  }
  if (discoveryAgeFilter === "action_queue") {
    return jobs.filter((job) => isDiscoveryActionQueueCandidate(job));
  }
  return jobs.filter(
    (job) =>
      getDiscoveryAgeTier(job) === discoveryAgeFilter &&
      getDeadlineUrgency(job) !== "expired",
  );
};
