...
export interface ExtractorLimits {
  ...
  ontarioPublicSectorMaxJobs: number;
...
export function deriveExtractorLimits(args: {
  ...
  const includesOntarioPublicSector = args.sources.includes(
    "ontario-public-sector",
  );
  const includesPolicyJobsOttawa = args.sources.includes("policyjobs-ottawa");
  ...
  const weightedContributors =
    ...
    (includesOntarioPublicSector || includesPolicyJobsOttawa ? 1 : 0) +
  ...
  if (weightedContributors <= 0) {
    return {
      ...
      ontarioPublicSectorMaxJobs: budget,
  ...
  return {
    ...
    ontarioPublicSectorMaxJobs: Math.min(budget, perUnit + remainder),
...
export function calculateAutomaticEstimate(args: {
  ...
  const hasOntarioPublicSector = sources.includes("ontario-public-sector");
  const hasPolicyJobsOttawa = sources.includes("policyjobs-ottawa");
  ...
  const hiringCafeCap = hasHiringCafe
    ...
    : 0;
  const ontarioPublicSectorCap =
    hasOntarioPublicSector || hasPolicyJobsOttawa
      ? limits.ontarioPublicSectorMaxJobs
      : 0;
