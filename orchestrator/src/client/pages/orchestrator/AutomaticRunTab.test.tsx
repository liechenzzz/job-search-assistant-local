...
describe("AutomaticRunTab", () => {
  ...
  it("shows PolicyJobsOTT in the source picker without extra clicks", () => {
    render(
      <AutomaticRunTab
        open
        settings={createAppSettings({
        ...
        enabledSources={["ontario-public-sector", "policyjobs-ottawa"]}
        pipelineSources={["ontario-public-sector", "policyjobs-ottawa"]}
        onToggleSource={vi.fn()}
        onSetPipelineSources={vi.fn()}
        isPipelineRunning={false}
        onSaveAndRun={vi.fn().mockResolvedValue(undefined)}
    ...
    expect(
      screen.getByRole("button", { name: "Review and edit sources" }),
    ).toHaveAttribute("aria-expanded", "true");
    expect(
      screen.getByRole("button", { name: "PolicyJobsOTT" }),
    ).toBeInTheDocument();
  });
  ...
  it("applies GTA city presets without manual typing", () => {
    render(
      <AutomaticRunTab
        open
        settings={createAppSettings({
        ...
        enabledSources={["ontario-public-sector", "linkedin"]}
        pipelineSources={["ontario-public-sector"]}
        onToggleSource={vi.fn()}
        onSetPipelineSources={vi.fn()}
        isPipelineRunning={false}
        onSaveAndRun={vi.fn().mockResolvedValue(undefined)}
    ...
    fireEvent.click(screen.getByRole("button", { name: "GTA + nearby" }));
    fireEvent.focus(screen.getByLabelText("Cities"));
    ...
    expect(
      screen.getByRole("button", { name: "Remove city Barrie" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Remove city Waterloo" }),
    ).toBeInTheDocument();
