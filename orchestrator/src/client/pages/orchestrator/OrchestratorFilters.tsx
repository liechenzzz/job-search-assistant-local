...
interface OrchestratorFiltersProps {
  ...
  onDiscoveryAgeFilterChange: (value: DiscoveryAgeFilter) => void;
...
const tabDescriptions: Partial<Record<FilterTab, string>> = {
  ...
  archived: "Expired and skipped jobs — kept for reference",
};
...
export const OrchestratorFilters: React.FC<OrchestratorFiltersProps> = ({
  ...
  const activeFilterCount = useMemo(
    () =>
      ...
      Number(discoveryAgeFilter !== "action_queue") +
                    ...
                    <CardContent className="space-y-3">
                      {activeTab === "discovered" && (
                        <div className="flex flex-wrap gap-2">
                          {discoveryAgeFilterOrder.map((option) => (
                            <Button
                              key={option}
                              type="button"
                              size="sm"
                              variant={
                                discoveryAgeFilter === option
                                  ? "default"
                                  : "outline"
                              }
                              onClick={() =>
