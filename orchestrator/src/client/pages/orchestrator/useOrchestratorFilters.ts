...
const allowedDiscoveryAgeFilters = discoveryAgeFilterOrder;
...
export const useOrchestratorFilters = () => {
  ...
  const discoveryAgeFilter = useMemo((): DiscoveryAgeFilter => {
    const raw = searchParams.get("age") ?? DEFAULT_DISCOVERY_AGE_FILTER;
    return allowedDiscoveryAgeFilters.includes(raw as DiscoveryAgeFilter)
      ? (raw as DiscoveryAgeFilter)
      : DEFAULT_DISCOVERY_AGE_FILTER;
  }, [searchParams]);
  ...
  const setDiscoveryAgeFilter = useCallback(
    (value: DiscoveryAgeFilter) => {
      setSearchParams(
        (prev) => {
          if (value === DEFAULT_DISCOVERY_AGE_FILTER) prev.delete("age");
          else prev.set("age", value);
          return prev;
        },
        { replace: true },
      );
