...
export const OrchestratorPage: React.FC = () => {
  ...
  const defaultDiscoveryCount = useMemo(() => {
    const actionQueueCount = jobs.filter((job) =>
      isDiscoveryActionQueueCandidate(job),
    ).length;
    return Math.min(actionQueueCount, DISCOVERY_ACTION_QUEUE_LIMIT);
  }, [jobs]);
  const displayedCounts = useMemo(
    () => ({ ...counts, discovered: defaultDiscoveryCount }),
    [counts, defaultDiscoveryCount],
  );
      ...
      >
        ...
        <section className="space-y-4">
          ...
          <OrchestratorFilters
            ...
            onDiscoveryAgeFilterChange={setDiscoveryAgeFilter}
