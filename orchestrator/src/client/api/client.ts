...
function designResumeVariantQuery(variant?: DesignResumeVariant): string {
  return variant ? `?variant=${encodeURIComponent(variant)}` : "";
}

export function getDesignResumeHtmlPreviewUrl(
  ...
  return `${API_BASE}/design-resume/resume-html${query ? `?${query}` : ""}`;
}

function getDesignResumeHtmlPreviewEndpoint(
  variant?: DesignResumeVariant,
  revision?: number,
): string {
  const params = new URLSearchParams();
  ...
  if (typeof revision === "number") params.set("revision", String(revision));
  const query = params.toString();
  return `/design-resume/resume-html${query ? `?${query}` : ""}`;
}
...
export async function getJobDocumentDiagnostics(
  ...
  return fetchApi<JobDocumentDiagnostics>(
    `/jobs/${encodeURIComponent(id)}/document-diagnostics?t=${Date.now()}`,
  );
}

export async function getJobResumeAlignment(
  id: string,
): Promise<ResumeAlignmentDetailResponse> {
  return fetchApi<ResumeAlignmentDetailResponse>(
    `/jobs/${encodeURIComponent(id)}/resume-alignment?t=${Date.now()}`,
  );
}
...
export async function getJobWordBlob(id: string): Promise<Blob> {
  return fetchBlobApi(`/jobs/${encodeURIComponent(id)}/word`);
}

export async function getJobHtmlBlob(id: string): Promise<Blob> {
  return fetchBlobApi(`/jobs/${encodeURIComponent(id)}/resume-html`);
}

export async function getDesignResumeHtmlPreview(
  variant?: DesignResumeVariant,
  revision?: number,
): Promise<string> {
  return fetchTextApi(getDesignResumeHtmlPreviewEndpoint(variant, revision));
}
...
export async function getDesignResume(
  ...
  return fetchApi<DesignResumeDocument>(
    `/design-resume${designResumeVariantQuery(variant)}`,
  );
}

export async function getDesignResumeStatus(
  variant?: DesignResumeVariant,
): Promise<DesignResumeStatusResponse> {
  return fetchApi<DesignResumeStatusResponse>(
    `/design-resume/status${designResumeVariantQuery(variant)}`,
  );
}

export async function importDesignResumeFromRxResume(
  variant?: DesignResumeVariant,
): Promise<DesignResumeDocument> {
  return fetchApi<DesignResumeDocument>(
    `/design-resume/import/rxresume${designResumeVariantQuery(variant)}`,
    {
      method: "POST",
    },
  );
...
export async function generateCompactOnePageDesignResume(): Promise<DesignResumeDocument> {
  return fetchApi<DesignResumeDocument>("/design-resume/compact-one-page", {
    method: "POST",
  });
}
...
export async function exportDesignResume(
  variant?: DesignResumeVariant,
): Promise<DesignResumeExportResponse> {
  return fetchApi<DesignResumeExportResponse>(
    `/design-resume/export${designResumeVariantQuery(variant)}`,
  );
}
...
export async function generateDesignResumePdf(
  ...
  return fetchApi<DesignResumePdfResponse>(
    `/design-resume/generate-pdf${designResumeVariantQuery(variant)}`,
    {
      method: "POST",
    },
  );
...
export async function getDesignResumePdfBlob(pdfUrl?: string): Promise<Blob> {
  const endpoint = pdfUrl?.startsWith(API_BASE)
    ? pdfUrl.slice(API_BASE.length)
    : (pdfUrl ?? "/design-resume/pdf");
  return fetchBlobApi(endpoint);
}
...
export async function getResumeReferenceScan(): Promise<ResumeReferenceScanResult | null> {
  return fetchApi<ResumeReferenceScanResult | null>(
    "/settings/resume-references",
  );
}
...
export async function saveResumeReferenceScan(
  ...
  return fetchApi<ResumeReferenceScanResult>("/settings/resume-references", {
    method: "POST",
