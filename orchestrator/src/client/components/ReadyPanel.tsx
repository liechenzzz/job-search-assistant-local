...
export const ReadyPanel: React.FC<ReadyPanelProps> = ({
  ...
  const wordFilename = `${safeFilenamePart(personName || "Unknown")}_${safeFilenamePart(job?.employer || "Unknown")}.docx`;
  const htmlFilename = `${safeFilenamePart(personName || "Unknown")}_${safeFilenamePart(job?.employer || "Unknown")}.html`;
  ...
  const handleOpenWordDraft = useCallback(() => {
    if (!job) return;
    void openJobWordDraft(job.id).catch((error) => {
      toast.error(
  ...
  const handleDownloadWordDraft = useCallback(() => {
    if (!job) return;
    void downloadJobWordDraft(job.id, wordFilename).catch((error) => {
      toast.error(
  ...
  const handleOpenHtmlResume = useCallback(() => {
    if (!job) return;
    void openJobHtmlResume(job.id).catch((error) => {
      toast.error(
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="pb-4 border-b border-border/40">
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          {/* Word is the primary editable artifact. */}
          <Button
            variant="outline"
          ...
          >
            ...
            <span className="truncate">View HTML</span>
          </Button>
