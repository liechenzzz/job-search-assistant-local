...
const CURRENT_ALIGNMENT_ENGINE_VERSION = "semantic-v3";

const alignmentStatusLabel = (status: string): string => {
  switch (status) {
    case "pass":
      return "Strong match";
    case "warning":
...
export const ResumeAlignmentPill: React.FC<{
  report: ResumeAlignmentReport | null;
}> = ({ report }) => {
  if (!report) return null;

  if (report.engineVersion !== CURRENT_ALIGNMENT_ENGINE_VERSION) {
    return (
      <StatusIndicator
        ...
        tooltip={
          <div className="space-y-1">
            <p className="text-xs font-medium">Resume alignment</p>
            <p className="text-[10px] opacity-80">
              Alignment report stale; regenerate resume content.
            </p>
          </div>
        }
  ...
  const tooltip = (
    <div className="space-y-1">
      ...
      {report.evidenceFit ? (
        <p className="text-[10px] opacity-80">
          Experience coverage: {alignmentStatusLabel(report.evidenceFit.status)}{" "}
          {report.evidenceFit.score}/100
        </p>
      ) : null}
      <p className="text-[10px] opacity-80">
        Generated resume alignment: {alignmentStatusLabel(report.status)}{" "}
        {report.score}/100
      </p>
      {report.missingRequired.length > 0 ? (
        <p className="text-[10px] opacity-80">
          Missing: {report.missingRequired.slice(0, 3).join("; ")}
        </p>
      ) : report.partialRequired.length > 0 ? (
        <p className="text-[10px] opacity-80">
          Partial: {report.partialRequired.slice(0, 3).join("; ")}
        </p>
      ) : (
        <p className="text-[10px] opacity-80">
          Required qualifications are covered.
        </p>
      )}
      {report.humanInputNeeded && report.humanInputNeeded.length > 0 ? (
        <p className="text-[10px] opacity-80">
          Needs input: {report.humanInputNeeded.slice(0, 3).join("; ")}
        </p>
      ) : null}
      {report.wordingGapsAfterAutoRewrite &&
      report.wordingGapsAfterAutoRewrite.length > 0 ? (
        <p className="text-[10px] opacity-80">
          Gap reason: evidence found but not written clearly:{" "}
          {report.wordingGapsAfterAutoRewrite.slice(0, 3).join("; ")}
        </p>
      ) : null}
      {report.evidenceFit?.noEvidenceRequired.length ? (
        <p className="text-[10px] opacity-80">
          Gap reason: no reference evidence:{" "}
          {report.evidenceFit.noEvidenceRequired.slice(0, 3).join("; ")}
        </p>
      ) : null}
      {report.alignmentSource === "ai_calibrated" ? (
        <p className="text-[10px] opacity-70">AI calibrated coverage check</p>
      ) : null}
    </div>
  );
  ...
  return (
    <StatusIndicator
      dotColor={statusDot}
      label={`Resume Match ${report.score}%`}
      className={cn("cursor-help", statusText)}
      tooltip={tooltip}
      tooltipClassName="max-w-sm"
    />
  );
