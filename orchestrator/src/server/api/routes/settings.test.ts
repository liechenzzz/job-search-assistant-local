...
describe.sequential("Settings API routes", () => {
  ...
  it("returns settings with defaults", async () => {
    ...
    expect(body.data.humanizerEnabled.value).toBe(true);
    expect(body.data.impactFramingEnabled.value).toBe(true);
    expect(body.data.roleFramingMode.value).toBe("auto");
    expect(body.data.manualRoleFamily.value).toBe(
      "public_sector_policy_economic_development",
    );
    expect(body.data.customRoleFramingInstructions.value).toBe("");
  });
  ...
  it("rejects invalid settings updates and persists overrides", async () => {
    ...
    const patchRes = await fetch(`${baseUrl}/api/settings`, {
      ...
      body: JSON.stringify({
        ...
        customRoleFramingInstructions: "Favor prototype evidence.",
      }),
    });
    const patchBody = await patchRes.json();
    expect(patchBody.ok).toBe(true);
    ...
    expect(patchBody.data.humanizerEnabled.value).toBe(false);
    expect(patchBody.data.impactFramingEnabled.value).toBe(false);
    expect(patchBody.data.roleFramingMode.value).toBe("manual");
    expect(patchBody.data.manualRoleFamily.value).toBe("ai_digital_strategy");
    expect(patchBody.data.customRoleFramingInstructions.value).toBe(
      "Favor prototype evidence.",
    );
  });
