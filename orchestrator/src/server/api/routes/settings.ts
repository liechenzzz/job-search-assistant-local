...
settingsRouter.get(
  "/resume-references",
  asyncRoute(async (_req: Request, res: Response) => {
    ok(res, await getResumeReferenceScan());
  }),
);
...
settingsRouter.post(
  ...
  asyncRoute(async (req: Request, res: Response) => {
    if (isDemoMode()) {
      return sendDemoBlocked(
        res,
        "Saving resume references is disabled in the public demo.",
        { route: "POST /api/settings/resume-references" },
      );
    }
    ok(res, await saveResumeReferenceScan(req.body));
  }),
);

settingsRouter.post(
  "/resume-references/ingest",
  asyncRoute(async (req: Request, res: Response) => {
    if (isDemoMode()) {
      return sendDemoBlocked(
        res,
        "Scanning resume references is disabled in the public demo.",
        { route: "POST /api/settings/resume-references/ingest" },
      );
    }
