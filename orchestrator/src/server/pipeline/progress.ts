...
export const progressHelpers = {
  ...
  crawlingComplete: (jobsFound: number) =>
    updateProgress({
      step: "importing",
      message: `Kept ${jobsFound} candidate jobs after source filtering; importing...`,
  ...
  importComplete: (created: number, skipped: number) =>
    updateProgress({
      step: "scoring",
      message: `Saved ${created} new records (${skipped} known duplicates). Scoring eligible jobs...`,
  ...
  complete: (discovered: number, processed: number) =>
    updateProgress({
      step: "completed",
      message: `Pipeline complete. Kept ${discovered} candidate jobs, processed ${processed}.`,
