...
export async function getJobListItems(
  ...
  const selection = {
    ...
    relevanceStatus: jobs.relevanceStatus,
    relevanceReason: jobs.relevanceReason,
    availabilityStatus: jobs.availabilityStatus,
    availabilityReason: jobs.availabilityReason,
    availabilityCheckedAt: jobs.availabilityCheckedAt,
    status: jobs.status,
    outcome: jobs.outcome,
    closedAt: jobs.closedAt,
    suitabilityScore: jobs.suitabilityScore,
    resumeAlignmentReport: jobs.resumeAlignmentReport,
    resumeServiceFitReport: jobs.resumeServiceFitReport,
    resumePositioningPlan: jobs.resumePositioningPlan,
  ...
  return rows.map((row) => ({
    ...
    relevanceStatus: row.relevanceStatus as JobRelevanceStatus | null,
    availabilityStatus: row.availabilityStatus as JobAvailabilityStatus | null,
...
async function insertJob(input: CreateJobInput): Promise<Job> {
  ...
  await db.insert(jobs).values({
    ...
    relevanceStatus: input.relevanceStatus ?? null,
...
/**
 * Get open discovered jobs whose posting availability should be rechecked.
 */
export async function getAvailabilityRecheckCandidates(
  limit = 100,
): Promise<Job[]> {
  const tenantId = getActiveTenantId();
  const rows = await db
    ...
    .where(
      and(
        eq(jobs.tenantId, tenantId),
        inArray(jobs.status, ["discovered", "processing", "ready", "applied"]),
        sql`(${jobs.availabilityCheckedAt} IS NULL OR ${jobs.availabilityCheckedAt} < datetime('now', '-24 hours'))`,
      ),
    )
    .orderBy(desc(jobs.discoveredAt))
    .limit(limit);
...
function mapRowToJob(row: typeof jobs.$inferSelect): Job {
  return {
    ...
    jdKeywordProfile: row.jdKeywordProfile ?? null,
    jdQualificationProfile: row.jdQualificationProfile ?? null,
    jdServiceValueBrief: row.jdServiceValueBrief ?? null,
    resumeAlignmentReport: row.resumeAlignmentReport ?? null,
    resumeServiceFitReport: row.resumeServiceFitReport ?? null,
    resumePositioningPlan: row.resumePositioningPlan ?? null,
    resumeTargetPagesOverride:
      row.resumeTargetPagesOverride === 1 || row.resumeTargetPagesOverride === 2
        ? row.resumeTargetPagesOverride
        : null,
