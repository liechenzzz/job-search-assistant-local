...
export async function buildJobChatPromptContext(
  ...
  const [baseSystemPrompt, stopSlopEnabled, applicationWritingInstructions] =
    await Promise.all([
      buildSystemPrompt(style, profile),
      isStopSlopEnabled(),
      buildApplicationWritingInstructionsForJob({
        title: job.title,
        employer: job.employer,
  ...
  const systemPrompt = [
    baseSystemPrompt,
    stopSlopEnabled ? STOP_SLOP_GHOSTWRITER_PROMPT : null,
    `Application writing strategy:\n${applicationWritingInstructions}`,
    buildCoverLetterPolicyInstructions(documentPolicy, profile),
  ]
    .filter(Boolean)
    .join("\n\n");
