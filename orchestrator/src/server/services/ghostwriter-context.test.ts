...
describe("buildJobChatPromptContext", () => {
  ...
  it("includes selected role-family guidance in Ghostwriter context", async () => {
    const job = createJob({
      id: "job-ctx-role",
      title: "AI Strategy Analyst",
      jobDescription:
        "Build generative AI workflow automation, RAG knowledge base tools, and validation checkpoints.",
    });
    vi.mocked(getJobById).mockResolvedValue(job);
    vi.mocked(getProfile).mockResolvedValue({});
    ...
    expect(context.systemPrompt).toContain(
      "Role framing: AI / digital strategy (auto).",
    );
    expect(context.systemPrompt).toContain(
      "Keep AI claims operational and specific",
    );
  });
