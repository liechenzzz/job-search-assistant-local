...
- Prefer projects that cover the required qualifications first, then job description keywords.
- Prefer projects that signal impact and real-world engineering.
- Do NOT invent projects or skills.

JD qualification profile:
${args.qualificationProfile ? JSON.stringify(args.qualificationProfile, null, 2) : "Not available"}
...
function fallbackPickProjectIds(
  ...
  const jd = [
    jobDescription || "",
    qualificationProfile?.required.join(" ") ?? "",
    qualificationProfile?.preferred.join(" ") ?? "",
    qualificationProfile?.keywords.join(" ") ?? "",
  ]
    .join(" ")
    .toLowerCase();
