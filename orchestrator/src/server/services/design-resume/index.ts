...
export const DEFAULT_DESIGN_RESUME_VARIANT: DesignResumeVariant = "two_page";
const DESIGN_RESUME_VARIANT_IDS: Record<DesignResumeVariant, string> = {
  one_page: "one_page",
  two_page: DESIGN_RESUME_DEFAULT_ID,
};
...
export function normalizeDesignResumeVariant(
  value: unknown,
): DesignResumeVariant {
  return value === "one_page" || value === "two_page"
    ? value
    : DEFAULT_DESIGN_RESUME_VARIANT;
}

function buildTenantScopedDesignResumeDocumentId(
  variant: DesignResumeVariant = DEFAULT_DESIGN_RESUME_VARIANT,
): string {
  return `${DESIGN_RESUME_VARIANT_IDS[variant]}_${getActiveTenantId()}`;
}
...
export async function replaceCurrentDesignResumeDocument(input: {
  ...
  const variant = input.variant ?? DEFAULT_DESIGN_RESUME_VARIANT;
...
export async function getCurrentDesignResume(
  variant: DesignResumeVariant = DEFAULT_DESIGN_RESUME_VARIANT,
): Promise<DesignResumeDocument | null> {
  try {
    return hydrateDocument(
      await designResumeRepo.getDesignResumeDocumentById(
        buildTenantScopedDesignResumeDocumentId(variant),
      ),
    );
  } catch (error) {
...
export async function getCurrentDesignResumeOrNullOnLegacy(
  variant: DesignResumeVariant = DEFAULT_DESIGN_RESUME_VARIANT,
): Promise<DesignResumeDocument | null> {
  try {
    return await getCurrentDesignResume(variant);
  } catch (error) {
...
export async function getDesignResumeForTargetPages(
  targetPages?: 1 | 2,
): Promise<DesignResumeDocument | null> {
  if (targetPages === 1) {
    return (
      (await getCurrentDesignResume("one_page")) ??
      (await getCurrentDesignResume("two_page"))
    );
  }
  return getCurrentDesignResume("two_page");
}

export async function requireCurrentDesignResume(
  variant: DesignResumeVariant = DEFAULT_DESIGN_RESUME_VARIANT,
): Promise<DesignResumeDocument> {
...
export async function importDesignResumeFromReactiveResume(
  variant: DesignResumeVariant = DEFAULT_DESIGN_RESUME_VARIANT,
): Promise<DesignResumeDocument> {
...
export async function updateCurrentDesignResume(
  ...
  const current = await requireCurrentDesignResume(variant);
