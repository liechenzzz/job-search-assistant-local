...
import * as rxresume from "./rxresume";
...
describe("PDF Service Tailoring Logic", () => {
  ...
  it("passes a fallback JD keyword profile into resume preparation", async () => {
    const result = await generatePdf(
      "job-domain-gate",
      {},
      "Build SQL reports and Power BI dashboards for operations teams.",
      undefined,
      "",
    );

    expect(rxresume.prepareTailoredResumeForPdf).toHaveBeenCalledWith(
      expect.objectContaining({
        tailoredContent: expect.objectContaining({
          jdKeywordProfile: expect.objectContaining({
    ...
    expect(result.jdKeywordProfile).toMatchObject({
      roleFamily: "data_analytics_operations",
    });
  });

  it("uses the current JD profile instead of stale stored tailoring profile", async () => {
    await generatePdf(
      "job-lululemon-strategy",
      {
        jdKeywordProfile: {
          roleFamily: "public_sector_policy_economic_development",
          requiredKeywords: ["policy analysis"],
          domainKeywordsPresent: ["NOC", "NAICS"],
          blockedUnlessPresent: [],
          experienceFocus: ["policy analysis"],
        },
      },
      ...
      "",
      {
        jobTitle: "Senior Analyst, Enterprise Strategy & Planning",
        jobEmployer: "lululemon",
      },
    );
    ...
    expect(rxresume.prepareTailoredResumeForPdf).toHaveBeenCalledWith(
      expect.objectContaining({
        tailoredContent: expect.objectContaining({
          jdKeywordProfile: expect.objectContaining({
            requiredKeywords: expect.arrayContaining(["strategy", "KPI"]),
  ...
  it("uses a one-page LaTeX layout without compacting resume content", async () => {
    await generatePdf("job-one-page-layout", {}, "desc", undefined, undefined, {
      resumeTargetPages: 1,
    });

    expect(mockResumeRenderer.renderResumePdf).toHaveBeenCalledWith(
      expect.objectContaining({
        jobId: "job-one-page-layout",
        layout: "one-page-compact",
      }),
    );
  });

  it("uses layout-only fallback passes for two-page LaTeX overflows", async () => {
    mocks.readFile.mockResolvedValue("/Type /Page /Type /Page /Type /Page");

    await generatePdf("job-two-page-layout", {}, "desc", undefined, undefined, {
      resumeTargetPages: 2,
    });

    const layouts = mockResumeRenderer.renderResumePdf.mock.calls.map(
      ([args]: any[]) => args.layout,
    );
