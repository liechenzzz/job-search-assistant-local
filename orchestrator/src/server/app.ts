...
const CLIENT_BUNDLE_REQUIRED_MARKERS = [
  "Resume Match ",
  "Strong match",
  "Partial match",
  "Weak match",
...
export function createApp() {
  ...
  // Folder reference scans include searchable text chunks for every resume and
  // cover letter in the selected directory, so they can exceed Express' default
  // JSON limit while still being normal local-only metadata.
  app.use("/api/settings/resume-references", express.json({ limit: "250mb" }));
  ...
  app.get("/health", (_req, res) => {
    ...
    res.json({
      ...
      alignmentEngineVersion: SEMANTIC_QUALIFICATION_ENGINE_VERSION,
      semanticQualificationRules: true,
      clientAsset: clientDir ? getNewestClientBundle(clientDir) : null,
    });
  });
  ...
  if (process.env.NODE_ENV === "production") {
    ...
    warnIfClientBundleIsStale(clientDir);
...
function warnIfClientBundleIsStale(clientDir: string): void {
  try {
    const assetsDir = join(clientDir, "assets");
    if (!existsSync(assetsDir)) {
      logger.warn("Frontend bundle check skipped: assets directory missing", {
        clientDir,
      });
      return;
    }

    const newestBundle = getNewestClientBundle(clientDir);

    if (!newestBundle) {
      logger.warn("Frontend bundle check skipped: index bundle missing", {
        assetsDir,
      });
      return;
    }

    const bundlePath = join(assetsDir, newestBundle);
    const bundleText = readFileSync(bundlePath, "utf8");
    const missingMarkers = CLIENT_BUNDLE_REQUIRED_MARKERS.filter(
      (marker) => !bundleText.includes(marker),
    );

    if (missingMarkers.length > 0) {
      logger.warn("Frontend bundle appears stale; rebuild client assets", {
        bundle: newestBundle,
        missingMarkers,
      });
    }
  } catch (error) {
    logger.warn("Frontend bundle check failed", { error });
  }
}

function getNewestClientBundle(clientDir: string): string | null {
  const assetsDir = join(clientDir, "assets");
