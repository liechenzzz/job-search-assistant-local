...
const migrations = [
  ...
  `CREATE TABLE IF NOT EXISTS jobs (
    ...
    jd_keyword_profile TEXT,
    jd_qualification_profile TEXT,
    jd_service_value_brief TEXT,
    resume_alignment_report TEXT,
    resume_service_fit_report TEXT,
    resume_positioning_plan TEXT,
    resume_target_pages_override INTEGER,
  ...
  `ALTER TABLE jobs ADD COLUMN relevance_status TEXT`,
  `ALTER TABLE jobs ADD COLUMN relevance_reason TEXT`,
  `ALTER TABLE jobs ADD COLUMN availability_status TEXT`,
  `ALTER TABLE jobs ADD COLUMN availability_reason TEXT`,
  ...
  `ALTER TABLE jobs ADD COLUMN jd_keyword_profile TEXT`,
  `ALTER TABLE jobs ADD COLUMN jd_qualification_profile TEXT`,
  `ALTER TABLE jobs ADD COLUMN jd_service_value_brief TEXT`,
  `ALTER TABLE jobs ADD COLUMN resume_alignment_report TEXT`,
  `ALTER TABLE jobs ADD COLUMN resume_service_fit_report TEXT`,
  `ALTER TABLE jobs ADD COLUMN resume_positioning_plan TEXT`,
  `ALTER TABLE jobs ADD COLUMN resume_target_pages_override INTEGER`,
  ...
  `INSERT OR REPLACE INTO jobs_new (
    ...
    deadline, salary, location, location_evidence, degree_required, starting, job_description, relevance_status, relevance_reason,
    availability_status, availability_reason, availability_checked_at, status, outcome, closed_at,
    suitability_score, suitability_reason, tailored_summary, tailored_headline, tailored_skills, tailored_experience, jd_keyword_profile,
    jd_qualification_profile, jd_service_value_brief, resume_alignment_report, resume_service_fit_report, resume_positioning_plan, resume_target_pages_override, selected_project_ids, pdf_path, tracer_links_enabled, sponsor_match_score, sponsor_match_names, discovered_at, processed_at,
    ready_at,
  ...
  SELECT
    ...
    ${tableHasColumn("jobs", "tailored_experience") ? "tailored_experience" : "NULL"},
    ${tableHasColumn("jobs", "jd_keyword_profile") ? "jd_keyword_profile" : "NULL"},
    ${tableHasColumn("jobs", "jd_qualification_profile") ? "jd_qualification_profile" : "NULL"},
    ${tableHasColumn("jobs", "jd_service_value_brief") ? "jd_service_value_brief" : "NULL"},
    ${tableHasColumn("jobs", "resume_alignment_report") ? "resume_alignment_report" : "NULL"},
    ${tableHasColumn("jobs", "resume_service_fit_report") ? "resume_service_fit_report" : "NULL"},
    ${tableHasColumn("jobs", "resume_positioning_plan") ? "resume_positioning_plan" : "NULL"},
    ${tableHasColumn("jobs", "resume_target_pages_override") ? "resume_target_pages_override" : "NULL"},
...
function ensureJobsTailoringColumns(): void {
  if (!tableExists("jobs")) return;

  const requiredColumns = [
    {
      name: "tailored_experience",
      sql: "ALTER TABLE jobs ADD COLUMN tailored_experience TEXT",
    },
    {
      name: "jd_keyword_profile",
      sql: "ALTER TABLE jobs ADD COLUMN jd_keyword_profile TEXT",
    },
    {
      name: "jd_qualification_profile",
      sql: "ALTER TABLE jobs ADD COLUMN jd_qualification_profile TEXT",
    },
    {
      name: "jd_service_value_brief",
      sql: "ALTER TABLE jobs ADD COLUMN jd_service_value_brief TEXT",
    },
    {
      name: "resume_alignment_report",
      sql: "ALTER TABLE jobs ADD COLUMN resume_alignment_report TEXT",
    },
    {
      name: "resume_service_fit_report",
      sql: "ALTER TABLE jobs ADD COLUMN resume_service_fit_report TEXT",
    },
    {
      name: "resume_positioning_plan",
      sql: "ALTER TABLE jobs ADD COLUMN resume_positioning_plan TEXT",
    },
    {
      name: "resume_target_pages_override",
      sql: "ALTER TABLE jobs ADD COLUMN resume_target_pages_override INTEGER",
    },
  ];

  for (const column of requiredColumns) {
    if (tableHasColumn("jobs", column.name)) continue;
    sqlite.exec(column.sql);
    console.log(`✅ Added missing jobs.${column.name} column`);
  }

  const missingColumns = requiredColumns
    .map((column) => column.name)
    .filter((columnName) => !tableHasColumn("jobs", columnName));
  if (missingColumns.length > 0) {
    console.error(
      `❌ Migration failed: jobs table is missing required tailoring columns: ${missingColumns.join(", ")}`,
    );
    process.exit(1);
  }
}

ensureJobsTailoringColumns();
