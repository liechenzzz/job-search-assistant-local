...
COPY extractors/ontario-public-sector/package*.json ./extractors/ontario-public-sector/
...
COPY extractors/ontario-public-sector ./extractors/ontario-public-sector
...
# Fail the image build if critical resume/RAG rules are missing from the
# production client bundle or server runtime source.
RUN set -eux; \
    grep -R "public_sector_government" /app/orchestrator/dist/client/assets; \
    grep -R "Government / public sector" /app/orchestrator/dist/client/assets; \
    grep -R "resumeTargetPagesOverride" /app/orchestrator/dist/client/assets; \
    grep -R "allowsManualResumeTargetPages" /app/orchestrator/dist/client/assets; \
    grep -R "Reference RAG ready" /app/orchestrator/dist/client/assets; \
    grep -R "resumeTargetPages: resumeDecision.targetPages" /app/orchestrator/src/server/pipeline/orchestrator.ts; \
    grep -R "SELECTED EVIDENCE GATE" /app/orchestrator/src/server/services/summary.ts; \
    grep -R "Reference index probe returned no searchable chunks" /app/orchestrator/src/server/services/resume-references.ts; \
    grep -R "SYSTEM SELECTED EVIDENCE BANK" /app/orchestrator/src/server/services/summary.ts; \
    grep -R "resume_evidence_rerank" /app/orchestrator/src/server/services/resume-evidence-rerank.ts; \
    grep -R "Evidence gate" /app/orchestrator/dist/client/assets
