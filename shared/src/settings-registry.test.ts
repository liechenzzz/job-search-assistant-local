...
describe("settingsRegistry helpers", () => {
  ...
  describe("string parsing (parseNonEmptyStringOrNull)", () => {
    ...
    it("accepts nearby GTA city preset strings", () => {
      const nearbyGtaCities = [
        "Toronto",
        "Mississauga",
        "Brampton",
        "Markham",
      ...
      expect(
        settingsRegistry.searchCities.schema.safeParse(nearbyGtaCities),
      ).toMatchObject({ success: true });
      expect(
        settingsRegistry.jobspyLocation.schema.safeParse(nearbyGtaCities),
      ).toMatchObject({ success: true });
    });
