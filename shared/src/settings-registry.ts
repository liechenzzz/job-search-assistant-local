...
import {
  ...
  PDF_RENDERER_VALUES,
  type PdfRenderer,
  RESUME_OUTPUT_MODE_VALUES,
  type ResumeOutputMode,
...
export const SEARCH_CITIES_MAX_LENGTH = 500;
...
const parseRoleFramingModeOrNull = createEnumParser(ROLE_FRAMING_MODE_VALUES);
const parseApplicationRoleFamilyOrNull = createEnumParser(
  APPLICATION_ROLE_FAMILY_VALUES,
);
const parsePdfRendererOrNull = createEnumParser(PDF_RENDERER_VALUES);
const parseResumeOutputModeOrNull = createEnumParser(RESUME_OUTPUT_MODE_VALUES);
...
export const settingsRegistry = {
  ...
  resumeOutputMode: {
    kind: "typed" as const,
    schema: z.enum(RESUME_OUTPUT_MODE_VALUES),
    default: (): ResumeOutputMode => "strict_ats_visual",
    parse: parseResumeOutputModeOrNull,
    serialize: (value: ResumeOutputMode | null | undefined): string | null =>
      value ?? null,
  },
  ukvisajobsMaxJobs: {
    kind: "typed" as const,
  ...
  ontarioPublicSectorMaxJobs: {
    kind: "typed" as const,
    schema: z.number().int().min(1).max(1000),
    default: (): number =>
      parseInt(
        typeof process !== "undefined"
          ? process.env.ONTARIO_PUBLIC_SECTOR_MAX_JOBS || "150"
          : "150",
        10,
      ),
    parse: parseIntOrNull,
  ...
  searchCities: {
    kind: "typed" as const,
    schema: z.string().trim().max(SEARCH_CITIES_MAX_LENGTH),
    default: (): string =>
      typeof process !== "undefined"
  ...
  roleFramingMode: {
    kind: "typed" as const,
    schema: z.enum(ROLE_FRAMING_MODE_VALUES),
    default: (): RoleFramingMode => "auto",
    parse: parseRoleFramingModeOrNull,
    serialize: (value: RoleFramingMode | null | undefined): string | null =>
      value ?? null,
  },
  manualRoleFamily: {
    kind: "typed" as const,
    schema: z.enum(APPLICATION_ROLE_FAMILY_VALUES),
    default: (): ApplicationRoleFamily =>
      "public_sector_policy_economic_development",
    parse: parseApplicationRoleFamilyOrNull,
    serialize: (
      value: ApplicationRoleFamily | null | undefined,
    ): string | null => value ?? null,
  },
